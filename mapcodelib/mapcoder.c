...
static int firstrec(int ccode)
{
    return data_start[ccode];
}

static int lastrec(int ccode)
{
...
static int isSubdivision(int ccode)
{
...
static const char *get_entity_iso3(char *entity_iso3_result, int ccode)
{
    if (ccode < 0 || ccode >= MAX_MAPCODE_TERRITORY_CODE)
        ccode = ccode_earth; // solve bad args
...
{
    if (s[0] == 0 || s[1] == 0)
        return -27; // solve bad args

    if (len != 2 && len != 3)
        return -923; // solve bad args
    ...
    for (char *t = country; *t != 0; t++)
        *t = (char) toupper(*t);

    char *f = strstr(p, country);
    if (f == NULL)
...
static int ccode_of_iso3(const char *in_iso, int parentcode)
{
    ...
    if (in_iso && in_iso[0] && in_iso[1]) {
        ...
        return -23; // solve bad args
    }

    // make (uppercased) copy of at most three characters
    iso[0] = (char) toupper(in_iso[0]);
    if (iso[0])
        iso[1] = (char) toupper(in_iso[1]);
    if (iso[1])
        iso[2] = (char) toupper(in_iso[2]);
    iso[3] = 0;
    ...
    if (iso[2] == 0 || iso[2] == ' ') { // 2-letter iso code?
        ...
        if (s == NULL || hyphenated) {
            ...
            while (a) {
                ...
                if (a && a[3] == '=' && (a[4] > '9' ||
                    a[4] == (char) (48 + parentcode) || parentcode < 0)) {
                        memcpy(iso, a + 4, 3);
                        a = NULL;
                            ...
                            const encodeRec *enc) // append extra characters to result for more precision
{
    ...
    if (extraDigits < 0)
        extraDigits = 0;
    else if (extraDigits > MAX_PRECISION_DIGITS)
...
static int decodeExtension(decodeRec *dec, int dividerx4, int dividery,
    int ydirection)
{
  ...
  if (*extrapostfix) {
    ...
    c2 = decodeChar(extrapostfix[1]) ? extrapostfix[1] : 72); // 72='H'=code 15=(3+2*6)
    if (c2 < 0)
        c2 = 0;
    else if (c2 > 29)
        c2 = 29;
...
#else // new floating-point version
    double dividerx = dividerx4 / 4.0;
    double processor = 1.0;
    dec->lon = 0;
    dec->lat = 0;
    while (*extrapostfix) {
        int c1 = decodeChar(*extrapostfix++);
        if (c1 < 0 || c1 == 30)
            return -1;  // illegal extension character
...
static int decodeGrid(decodeRec *dec, int m, int hasHeaderLetter)
{
    ...
    if (prelen <= MAXFITLONG) {
        ...
        if (divx != divy && prelen > 2) { // D==6 special grid, useful when prefix is 3 or more, and not a nice 961x961
            decodeSixWide(v, divx, divy, &relx, &rely); // DECODE
        } else {
            relx = (v / divy);
            rely = divy - 1 - (v % divy);
        }
    ...
    // and then encode relative to THE CORNER of this cell
...
static void encodeGrid(char *result, const encodeRec *enc, int const m,
    int extraDigits, char headerLetter)
{
...
static int decodeNameless(decodeRec *dec, int m)
{
    ...
    int err = decodeExtension(dec, dividerx4, dividery, -1); // nameless
...
{ // rewrite all-digit codes
    ...
    if (dotpos) {
        ...
        if (*s == 'a' || *s == 'e' || *s == 'u' ||
            *s == 'A' || *s == 'E' || *s == 'U') { // thus, all digits, s[2]=0, after dot
...
static int encodeNameless(char *result, const encodeRec *enc,
    int input_ctry, int extraDigits, int m)
{
...
static int decodeAutoHeader(decodeRec *dec, int m)
{
    ...
    int value = decodeBase31(input); // decode top
    value *= (961 * 31);
...
static void encoderEngine(int ccode, const encodeRec *enc, int stop_with_one_result,
    int extraDigits, int result_override)
{
    if (enc == NULL || ccode < 0 || ccode > ccode_earth)
        return; // bad arguments

    int from = firstrec(ccode);
    int upto = lastrec(ccode);
    int x = enc->lon32;
    int y = enc->lat32;
    ...
    for (int i = from; i <= upto; i++) {
        if (coDex(i) < 54) {
            if (fitsInside(x, y, i)) {
                ...
                // if the last item is a reference to a state's country
                } else if (i == upto && isRestricted(i) && isSubdivision(ccode)) {
...
static int decoderEngine(decodeRec *dec)
{
    ...
    while (len > 0 && r[len - 1] >= 0 && r[len - 1] <= 32)
        len--; // remove trail
    memcpy(s, r, len);
    s[len] = 0;
    ...
    // assume ISO3 space|minus ISO23 space MAPCODE
    if (len > 8 && (s[3] == ' ' || s[3] == '-') && (s[6] == ' ' || s[7] == ' ')) {
        ...
        // assume ISO2 space|minus ISO23 space MAPCODE
        if (len > 7 && (s[2] == ' ' || s[2] == '-') && (s[5] == ' ' || s[6] == ' ')) {
    ...
    // assume ISO whitespace MAPCODE, overriding iso3
    if (len > 4 && s[3] == ' ') {
...
//////////// AT THIS POINT, dot=FIRST DOT, input=CLEAN INPUT (no vowels) ilen=INPUT LENGTH
    ...
    // analyse input
    int prelen = (int) (dot - s);
    int postlen = len - 1 - prelen;
    int codex = prelen * 10 + postlen;
...
static UWORD asc2lan[MAX_LANGUAGES][36] = {
    ...
    for (; *s != 0 && w < e; s++) {
        if (*s >= 1 && *s <= 'z') { // normal ascii
            *w++ = (char) (*s);
            ...
            int found = 0;
            for (int i = 0; unicode2asc[i].min != 0; i++) {
                                           ...
                                           int extraDigits) // 1.31 allow to stop after one result
{
        ...
        for (; ;) {
            ...
            if (v2 >= 0 && v2 < 1024) { // leaf?
                ...
                for (j = 0; j <= nr; j++) {
                    ...
                    if ((stop_with_one_result || debugStopAt >= 0) && enc.mapcodes->count > 0) {
                        break;
                    }
                }
                break;
...
#else
        for(int i = 0; i < MAX_MAPCODE_TERRITORY_CODE ; i++) {
    ...
    if (v) {
        for (int i = 0; i < enc.mapcodes->count; i++) {
...
int convertTerritoryIsoNameToCode(const char *string, int optional_tc) // optional_tc: pass 0 or negative if unknown
{
    ...
    if (ccode < 0 || strchr(string, '-') || strlen(string) > 3) {
        ...
        // there is a ccode, there is no hyphen in the string, and the string is as most 3 chars
        char tmp[12];
...
// context_tc is used to disambiguate ambiguous short mapcode inputs; pass 0 or negative if not available
int decodeMapcodeToLatLon(double *lat, double *lon, const char *input,  int context_tc)
{
